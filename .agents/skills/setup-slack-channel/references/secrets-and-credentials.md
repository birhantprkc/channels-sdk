...
| Slack **app-level** token | `xapp-…` (needs `connections:write`) | The Slack app, Basic Information → App-Level Tokens | The Slack adapter form in Intelligence, typed by the developer | In `.env`, in the repo, in chat |
...
The load-bearing split: **Slack tokens go to Intelligence, not to your app.** A
managed Channel holds no platform credentials. If you find yourself adding
...
> I don't need those and shouldn't have them. The `xoxb-` and `xapp-` tokens go
> straight into the Slack adapter form in Intelligence, entered by you. I'll
> tell you exactly which field each one goes in.
...
- Reinstalling a Slack app to add a scope issues a new bot token. The token
  already stored in Intelligence becomes stale and must be re-entered.
